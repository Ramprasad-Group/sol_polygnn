...
class PseudoDC(StandardMpModule):  # Pseudo DeepChem layer
    def __init__(self, node_size, E, hps, debug=False):
        ...
        # set up V
        self.V = st.layers.Mlp(self.node_size, self.node_size, self.hps, self.debug)
        # set up U
        self.U = st.layers.Mlp(
...
class MessagingPassingX0(st.std_module.StandardModule):
    def __init__(
        self,
        node_size,
        edge_size,
        ...
        self.R = st.layers.my_hidden2(
            self.node_size,
            self.readout_dim,
            hps=self.hps,
        )
        # set up message passing layers
        self.E = st.layers.Mlp(
            edge_size,
            edge_size,
            hps=self.hps,
            debug=self.debug,
...
class MtConcat_PolyMpnn(MessagingPassingX0):
    def __init__(
        self, node_size, edge_size, selector_dim, hps, normalize_embedding, debug
    ):
        super().__init__(
