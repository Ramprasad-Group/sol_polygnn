...
from sol_polygnn.trimer import TrimerLinear, TrimerLadder
from . import constants


import random
import sol_polykit.sol_polykit as spk
...
def multiply_polymer(smile, stoch=True, force_multiply=None):
    ...
    if stoch:
        ...
        n_repeat = 1
    if "[g]" in smile:
        polymer_class = spk.LadderPolymer
    else:
        polymer_class = spk.LinearPol
...
def multiply_and_cyclize(smile, n_repeat):
    ...
    try:
        ...
        print(f"\nThis polymer failed {smile}. Its type is {type(smile)}.", flush=True)
        raise e
...
def get_equivalent_graph_tensors(
    smile,
    upper_bound,
    bond_config,
    atom_config,
    ...
    """
    Keyword arguments:
        smile (list): List of smiles strings.
        upper_bound (int): The largest number of multiplications to use
            for each smiles string
        bond_config
        atom_config
        combiner (str): When there are multiple smiles strings, each string
            will have a different set of valid multiplications (e.g.,
            [*]C[*] and [*]CO[*]). So, there are several ways that one
            can permute these two sets. The valid options include:
            "intersection", "random", "first_n".
    """
    random.seed(random_seed)
    # coerce smile to a list
    if isinstance(smile, str):
        smile = [smile]
    valid_n_repeats = [get_valid_multiplications(sm, upper_bound) for sm in smile]
    if len(smile) == 1:
        valid_n_repeats = valid_n_repeats[0]
        return [
            construct_polymer_graph_monocycle(
                multiply_and_cyclize(smile[0], n_repeat), bond_config, atom_config
            )
...
def get_minimum_graph_tensor(smile, bond_config, atom_config, representation):
    ...
    if representation == "monocycle":
        n_repeat = [get_valid_multiplications(sm, 3)[0] for sm in smile]
        if len(smile) == 1:
            data = construct_polymer_graph_monocycle(
                multiply_and_cyclize(smile[0], n_repeat[0]), bond_config, atom_config
            )
        if len(smile) == 2:
            dataA = construct_polymer_graph_monocycle(
                multiply_and_cyclize(smile[0], n_repeat[0]), bond_config, atom_config
            )
            dataB = construct_polymer_graph_monocycle(
                multiply_and_cyclize(smile[1], n_repeat[1]), bond_config, atom_config
            )
            data = PairDataFromSoloData(dataA, dataB)
    elif representation == "trimer":
        polymer = []
        trimer_class = []
        for sm in smile:
            if "[g]" in sm:
                polymer.append(spk.LadderPolymer(sm))
                trimer_class.append(TrimerLadder)
            else:
                polymer.append(spk.LinearPol(sm))
                trimer_class.append(TrimerLinear)
        if len(polymer) == 1:
            data = construct_polymer_graph_trimer(
                polymer[0],
                bond_config,
                atom_config,
                trimer_class[0],
            ...
            dataA = construct_polymer_graph_trimer(
                polymer[0],
                bond_config,
                atom_config,
                trimer_class[0],
            )
            dataB = construct_polymer_graph_trimer(
                polymer[1],
                bond_config,
                atom_config,
                trimer_class[1],
...
def get_graph_tensor_debug_dimer(sm, bond_config, atom_config, representation):
    ...
    *Warning!* This function is only meant to be used for debugging purposes.
    ...
    if representation == "monocycle":
        ...
        if "[g]" in sm:
            polymer = spk.LadderPolymer(sm).multiply(2)
            trimer_class = TrimerLadder
        else:
            polymer = spk.LinearPol(sm).multiply(2)
